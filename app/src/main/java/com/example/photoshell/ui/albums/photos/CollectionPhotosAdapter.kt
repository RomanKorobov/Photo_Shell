...
class CollectionPhotosAdapter(private val onItemClicked: (id: String) -> Unit) :
    RecyclerView.Adapter<CollectionPhotosAdapter.Holder>() {
    ...
    class Holder(view: View, onItemClicked: (id: String) -> Unit) : RecyclerView.ViewHolder(view) {
        ...
        fun bindViewHolder(photo: UnsplashPhoto) {
            Glide.with(itemView).load(photo.urls.small).placeholder(R.drawable.ic_placeholder)
                .into(preview)
            theId = photo.id
        }
